...
dirs=(
    ["$HOME/projects/raspberry_pi/eye_tracking_robot/controlling_robot"]="~/scripts"
)
### Usage:
### dirs=(
...
for src in ${!dirs[@]} ; do
    rsync  -Pate ssh --exclude={.git,send_to_remote.sh} "$src" "$USER_CONN:${dirs[$src]}" # mirror directories
done
